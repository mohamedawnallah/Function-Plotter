...
@pytest.mark.qt
@pytest.mark.parametrize("function_input, xmin_input, xmax_input, expected_x_end, expected_y_end", [
    ("1/x","1", "10", 10, 0.1),
    ("sin(x)", "-1", "1", 1, 0.84),
    ("5*x^3 + 2*x", "1", "10", 10, 5020)
])
def test_plot_valid_functions(qtbot, function_plotter: FunctionPlotter, function_input, xmin_input, xmax_input, expected_x_end, expected_y_end):
    ...
    x_end_found, y_end_found = round(float(x[-1]),2), round(float(y[-1]),2)
    assert x_end_found == expected_x_end and y_end_found == expected_y_end, "The plotted function is not as expected."
...
def test_get_derivative(qtbot, function_plotter: FunctionPlotter):
    ...
    qtbot.keyClicks(function_plotter.xmax_input, xmax_input)
    qtbot.mouseClick(function_plotter.plot_button, Qt.LeftButton)
    derivative = function_plotter.get_derivative(message_timeout_seconds=message_timeout_seconds)
...
def test_get_integral(qtbot, function_plotter: FunctionPlotter):
    ...
    qtbot.keyClicks(function_plotter.xmax_input, xmax_input)
    qtbot.mouseClick(function_plotter.plot_button, Qt.LeftButton)
    integral = function_plotter.get_integral(message_timeout_seconds)
...
def test_change_color(qtbot, function_plotter: FunctionPlotter):
    ...
    function_plotter.change_color(color_input=color,message_timeout_seconds=message_timeout_seconds)
...
def test_toggle_grid(qtbot, function_plotter: FunctionPlotter):
    ...
    qtbot.keyClicks(function_plotter.xmax_input, xmax_input)
    qtbot.mouseClick(function_plotter.plot_button, Qt.LeftButton)
    function_plotter.toggle_grid(message_timeout_seconds=message_timeout_seconds)
    assert (
...
def test_toggle_legend(qtbot, function_plotter: FunctionPlotter):
    ...
    qtbot.keyClicks(function_plotter.xmax_input, xmax_input)
    qtbot.mouseClick(function_plotter.plot_button, Qt.LeftButton)
    function_plotter.toggle_legend(message_timeout_seconds=message_timeout_seconds)
    assert (
...
def test_add_another_function(qtbot, function_plotter: FunctionPlotter):
    ...
    function_plotter.plot_another_function(message_timeout_seconds=message_timeout_seconds)
    assert (
